...
function maf_sort(b,a,c){var e=window[b],d=$(a);b=d.find('i[class*="glyphicons"]');a="glyphicons glyphicons-chevron-down"==b.attr("class");var g=$("#"+c+" th").index(d),f=[];$("#"+c+" tr").each(function(a){var b=$(this).attr("id");b&&(a={},a.id=b,b=$(this).find("td:eq("+g+")").text(),a.value=b,f.push(a))});f.sort(function(a,b){var c=a.id.toLowerCase(),d=b.id.toLowerCase();return c<d?-1:c>d?1:0});f.sort(function(a,b){return e(a,b)});a&&f.reverse();c=$("#"+c+" tbody");for(d=0;d<f.length;d++){var h=
$("#"+f[d].id);h.remove();c.prepend(h)}a?b.removeClass("glyphicons-chevron-down").addClass("glyphicons-chevron-up"):b.removeClass("glyphicons-chevron-up").addClass("glyphicons-chevron-down")}function maf_stringSort(b,a){var c=b.value.toLowerCase(),e=a.value.toLowerCase();return c<e?-1:c>e?1:0}function maf_numberSort(b,a){return b.value-a.value}
...
function maf_filter_sort_url(b,a,c){if(a){var e=a.userColumnConfiguration[b].sortType;if(e&&"NONE"!=e){"UNSORTED"==e&&(e=a.userColumnConfiguration[b].sortType="DESC");var d=$(column).find('i[class*="glyphicons"]');"glyphicons glyphicons-sort"==d.attr("class")&&("DESC"==e?d.removeClass("glyphicons-sort").addClass("glyphicons-chevron-up"):d.removeClass("glyphicons-sort").addClass("glyphicons-chevron-down"));"glyphicons glyphicons-chevron-down"==d.attr("class")?(a.userColumnConfiguration[b].sortType=
"ASC",d.removeClass("glyphicons-chevron-down").addClass("glyphicons-chevron-up")):(a.userColumnConfiguration[b].sortType="DESC",d.removeClass("glyphicons-chevron-up").addClass("glyphicons-chevron-down"));c.apply()}}}
...
function maf_filter_setSortOrder(b,a,c){"UNSORTED"==c.userColumnConfiguration[a].sortType&&(b.removeClass("glyphicons-chevron-up"),b.removeClass("glyphicons-chevron-down"));"DESC"==c.userColumnConfiguration[a].sortType&&(b.removeClass("glyphicons-chevron-up"),b.hasClass("glyphicons-chevron-down")||b.addClass("glyphicons-chevron-down"));"ASC"==c.userColumnConfiguration[a].sortType&&(b.removeClass("glyphicons-chevron-down"),b.hasClass("glyphicons-chevron-up")||b.addClass("glyphicons-chevron-up"))}
...
function maf_filter_addSelect(b,a,c,e,d,g){var f=maf_filter_construct_field_header(a,e);f.append("<br/>");var h=$("<a/>",{href:"#",id:a});f.append(h);$("#"+b).append(f);var f=[],h=[],k=0,l=0;for(key in e.values)f[l]={},f[l].value=e.values[key].value,f[l].text=e.values[key].name,f[l].order=e.values[key].order,0<=e.defaultValue.indexOf(e.values[key].value)&&(h[k]=e.values[key].value,k++),l++;f.sort(maf_orderSort);$("#"+b).find("#"+a).editable({title:e.label,type:"checklist",placement:"bottom",value:h,
source:f,success:function(b,c){d.userColumnConfiguration[a].filterValue=c;d.currentPage=0;g.apply()}});maf_filter_add_remove_event(b,a,c,d,g)}
function maf_filter_addAutocomplete(b,a,c,e,d,g){var f=maf_filter_construct_field_header(a,e);f.append('<div class="clearfix"></div>');var h=$("<a/>",{href:"#",id:"_"+a+"_editable"});f.append(h);var h=$("<input/>",{type:"hidden",name:a,id:a,value:""}),k=$("<input/>",{type:"hidden",name:a+"_content",id:a+"_content",value:""});f.append(h);f.append(k);h=$("<a/>",{href:"#",id:"_"+a+"_clear_editable","class":"btn btn-default btn-sm"});h.append($("<span/>",{"class":"glyphicons glyphicons-erase"}));f.append(" &nbsp;");
f.append(h);$("#"+b).append(f);$("#"+b).find("#_"+a+"_clear_editable").click(function(c){c.preventDefault();$("#"+b).find("#_"+a+"_editable").editable("setValue",null);$("#"+b).find("#"+a).val(null);$("#"+b).find("#"+a+"_content").val("");d.userColumnConfiguration[a].filterValue={value:null,content:""};d.currentPage=0;g.apply()});window[a+"_editableCache"]={};""!=e.defaultValue.content&&(window[a+"_editableCache"][e.defaultValue.value]={name:e.defaultValue.content});maf_activateEditable_for_autocomplete(window[a+
"_editableCache"],b,a,e.defaultValue.value,"",e.url,"",function(b,c){d.userColumnConfiguration[a].filterValue={value:b,content:c};d.currentPage=0;g.apply()});maf_filter_add_remove_event(b,a,c,d,g)}
function maf_filter_construct_field_header(b,a){var c=$("<li/>",{"class":"filter-input",id:"_filter_cp_"+b}),e=$("<div/>",{"class":"pull-left"}),d=$("<div/>",{"class":"pull-right"}),g=$("<a/>",{href:"#",id:"_filter_remove_"+b,html:"<span class='glyphicons glyphicons-remove-2'></span>"}),f=$("<strong/>",{text:a.label});e.append(f);d.append("&nbsp;&nbsp;");d.append(g);c.append(e);c.append(d);return c}
...
a&&(d=!0);if(!d)return{newValue:k.find("#"+c+"_content").val()}},success:function(a,d){for(value in b)if(b[value].name==d){k.find("#"+c).val(value);k.find("#"+c+"_content").val(d);h&&h.apply(this,[value,d]);break}}})};